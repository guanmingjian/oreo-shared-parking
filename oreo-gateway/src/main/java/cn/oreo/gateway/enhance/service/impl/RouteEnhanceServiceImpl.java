...
import cn.oreo.gateway.enhance.entity.*;
import cn.oreo.gateway.enhance.service.*;
...
public class RouteEnhanceServiceImpl implements RouteEnhanceService {
    ...
    public Mono<Void> filterBlackList(ServerWebExchange exchange) {
        ...
        try {
            ...
            if (originUri != null) {
                String requestIp = OreoUtil.getServerHttpRequestIpAddress(request);
                ...
                if (forbid.get()) {
                    return OreoUtil.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
                            HttpStatus.NOT_ACCEPTABLE, new OreoResponse().message("黑名单限制，禁止访问"));
                }
            } else {
    ...
    public void saveRequestLogs(ServerWebExchange exchange) {
        ...
        if (!StringUtils.equalsIgnoreCase(TOKEN_CHECK_URL, originUri.getPath())) {
            ...
            ServerHttpRequest request = exchange.getRequest();
            String ipAddress = OreoUtil.getServerHttpRequestIpAddress(request);
    ...
    private Mono<Void> doRateLimitCheck(AtomicBoolean limit, RateLimitRule rule, URI uri,
        ...
        if (limit.get()) {
            ...
            if (count == 0) {
                ...
                return OreoUtil.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
                        HttpStatus.TOO_MANY_REQUESTS, new OreoResponse().message("访问频率超限，请稍后再试"));
            } else {
