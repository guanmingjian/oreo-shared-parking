...
import cn.oreo.auth.properties.OreoValidateCodeProperties;
import cn.oreo.auth.service.ValidateCodeService;
import cn.oreo.common.core.entity.constant.ImageTypeConstant;
...
public class ValidateCodeServiceImpl implements ValidateCodeService {
    ...
    public void create(HttpServletRequest request, HttpServletResponse response) throws IOException, ValidateCodeException {
        ...
        OreoValidateCodeProperties code = properties.getCode();
        setHeader(response, code.getType());

        Captcha captcha = createCaptcha(code);
        redisService.set(OreoConstant.CODE_PREFIX + key, StringUtils.lowerCase(captcha.text()), code.getTime());
        captcha.out(response.getOutputStream());
    }

    @Override
    public void check(String key, String value) throws ValidateCodeException {
        Object codeInRedis = redisService.get(OreoConstant.CODE_PREFIX + key);
    ...
    private Captcha createCaptcha(OreoValidateCodeProperties code) {
